...
export default function LPAgentSettings({ projectId }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* ── 6.5 Pending Chief Review queue (Mark SOP Chief Checkpoint) ── */}
      {(() => {
        const pending = safeRecentGenerations.filter(lp => lp.pending_review || lp.expired_review);
        ...
        return (
          <div className="card p-5">
            <h3 className="text-[13px] font-semibold text-textdark mb-3 flex items-center gap-2">
              ...
              Pending Chief Review
              <span className="ml-1 text-[10px] font-medium text-gold bg-gold/10 px-2 py-0.5 rounded-full">{pending.length}</span>
            </h3>

            <div className="space-y-1.5">
              {pending.map(lp => {
                const isExpired = !!lp.expired_review;
                const isStale = !!lp.stale_review && !isExpired;
                const hours = typeof lp.hours_in_review === 'number' ? lp.hours_in_review : null;
                const waitLabel = hours == null
                  ? ''
                  : hours < 1 ? `${Math.max(1, Math.round(hours * 60))}m`
                  : hours < 48 ? `${Math.round(hours)}h`
                  : `${Math.floor(hours / 24)}d`;
                const href = `/admin/projects/${projectId}/lp?lp=${lp.id}`;
                ...
                return (
                  <a
                    key={lp.id}
                    href={href}
                    className="flex items-center gap-3 rounded-lg border border-black/5 bg-white px-3 py-2 hover:border-navy/20 hover:bg-navy/[0.02] transition-colors"
                    title={lp.name}
                  >
                    <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${badgeClass}`}>
                      {isExpired ? 'expired' : isStale ? 'stale' : 'awaiting review'}
                    </span>
                    <span className="flex-1 text-[12px] font-medium text-textdark truncate">{lp.name}</span>
                    {waitLabel && (
                      <span className="text-[10px] text-textlight font-mono">{waitLabel}</span>
                    )}
            ...
            <p className="text-[10px] text-textlight mt-2">
              Reviews stale after 72h, auto-expire at 7 days. Expired LPs must be restored to pending review before they can publish.
            </p>
          </div>
        );
      })()}
