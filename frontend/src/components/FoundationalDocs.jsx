...
// ─── Headline Reference Document Section ───────────────────────────────────
function HeadlineReferenceSection({ projectId }) {
  ...
  const loadHeadlineDoc = async () => {
    try {
      const doc = await api.getHeadlineReference(projectId);
      setHeadlineDoc(doc);
    } catch (err) {
  ...
  const handleSave = async () => {
    if (!pasteContent.trim()) {
      toast.error('Please enter or upload headline content');
      return;
    }
    setSaving(true);
    try {
      await api.uploadHeadlineReference(projectId, pasteContent.trim());
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await api.deleteHeadlineReference(projectId);
      setHeadlineDoc(null);
  ...
  return (
    <div className="mt-8 pt-6 border-t border-gray-200/60">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h3 className="text-[14px] font-semibold text-gray-900">Headline Reference Document</h3>
        <span className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-orange-100 text-orange-700">Optional</span>
        <InfoTooltip text="Upload a document with headline examples (swipe file, headline collection, etc.). When the Headline Juicer is enabled in Ad Studio, the AI uses these as creative inspiration for more diverse, varied headlines." />
      </div>
      <p className="text-[12px] text-gray-500 mb-4 ml-8">
        Upload a swipe file or headline collection. The AI will use these as creative fuel — not copying directly, but drawing inspiration for more varied and punchy headlines.
      </p>

      {/* Existing document card */}
      {headlineDoc && !editing ? (
        <div className="ml-8 bg-white rounded-lg shadow-sm border border-orange-200 p-4">
          <div className="flex items-start justify-between mb-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] text-gray-400">v{headlineDoc.version}</span>
            </div>
            <div className="flex items-center gap-2">
              <button
                onClick={() => { setEditing(true); setPasteContent(headlineDoc.content || ''); }}
          ...
          <div className="flex items-center gap-3 mb-2 text-[11px] text-gray-400">
            ...
            {headlineDoc.updated_at && (
              <span>Updated {formatDate(headlineDoc.updated_at)}</span>
            )}
          </div>
          <button
        ...
        <div className="ml-8 space-y-3">
          ...
          />
          {pasteContent && (
            <p className="text-[11px] text-gray-400">{pasteContent.length.toLocaleString()} characters</p>
          )}
          <div className="flex items-center gap-2">
            <button
              onClick={handleSave}
            ...
            {editing && (
              <button
                onClick={() => { setEditing(false); setPasteContent(''); }}
                className="btn-secondary text-[13px] px-4 py-1.5"
              >
                Cancel
              </button>
            )}
...
export default function FoundationalDocs({ projectId, projectStatus }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {!editingDoc && !viewDoc && (
        <HeadlineReferenceSection projectId={projectId} />
      )}
