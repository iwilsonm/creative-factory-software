...
- Make headlines varied — use different techniques for each variation
- Write headlines that could work as Facebook ad hooks, VSL openers, or email subject lines

Output: Return ONLY a valid JSON array of objects, no markdown code fences:
[
  { "quote_index": 0, "headlines": ["Headline 1", "Headline 2", "Headline 3"] },
  { "quote_index": 1, "headlines": ["Headline 1", "Headline 2", "Headline 3", "Headline 4"] },
  ...
]`;
}
  ...
  for (let batchStart = 0; batchStart < quotes.length; batchStart += BATCH_SIZE) {
    ...
    const response = await withRetry(
      () => client.messages.create({
        model: 'claude-sonnet-4-6',
        max_tokens: 16000,
        system: 'You are an expert direct response copywriter. For each quote provided, generate 3-5 powerful, scroll-stopping headline variations. Always return output as a valid JSON array of objects.',
        messages: [
          { role: 'user', content: prompt }
        ],
      }),
    ...
    for (const result of batchResults) {
      if (result && typeof result.quote_index === 'number' && Array.isArray(result.headlines)) {
        allResults.push({
          ...
          headlines: result.headlines.filter(h => typeof h === 'string' && h.trim().length > 0),
        });
      }
    }
