...
CRITICAL: The final HTML template has placeholder tags (e.g., {{${autoContext.templateSlots[0]}}}) that will be replaced with the content you provide here. If you skip any of these sections, the finished page will display raw {{placeholder}} tags to the reader. Include ALL of them.` : ''}`;
  ...
  return { sections: validSections, autoGeneratedAuthor: Object.keys(autoGeneratedAuthor).length > 0 ? autoGeneratedAuthor : null };
}
 ...
 */
export function postProcessLP(html, { project = null, agentConfig = null, angle = '', editorialPlan = null, autoGeneratedAuthor = null } = {}) {
  let processed = html;
...
export async function generateAutoLP({
  ...
  audit('copy', 'generated', `${copySections.length} sections: ${copySections.map(s => s.type).join(', ')}${autoGeneratedAuthor ? `, author: ${autoGeneratedAuthor.name}` : ''}`);
