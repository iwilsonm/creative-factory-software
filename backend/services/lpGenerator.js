...
const LEGACY_LP_SOP_FOUNDATIONAL_PROMPT = `You are a world-class direct response copywriter. I'm attaching four foundational documents for a product I'm selling: Research, Avatar, Offer Brief, and Necessary Beliefs. Before we write anything, you must deeply analyze these documents and produce a structured PRE-WRITE MAP.

Your pre-write map must include:

1. BELIEF EXTRACTION — List every necessary belief from the Necessary Beliefs document. For each belief, note the strongest proof point or emotional trigger available in the research.

2. AVATAR EMOTIONAL JOURNEY — Map the reader's psychological arc through these four stages:
   • Awareness: What does the avatar already know/feel about the problem?
   • Frustration: What have they tried that failed? What makes this problem maddening?
   • Desperation: What is the breaking point — the moment they'd try anything?
   • Relief: What does life look like after the product solves this? Paint the after-state vividly.

3. SPECIFICITY ANCHORS — Extract concrete details from the research: product mechanisms, clinical data, ingredient names, measurable outcomes, timelines, percentages. These are the "proof atoms" that make copy believable instead of generic.

4. TRUST ARCHITECTURE — Identify every trust element available: testimonials, credentials, expert endorsements, guarantees, social proof, before/after evidence, certifications. Note which ones are strongest for this avatar.
 ...
 * @param {number} [params.wordCount] - Optional target word count
...
AUTHOR METADATA: Generate an appropriate author name and title for this article's byline and include:
  ...
  if (!hasWordCountTarget) {
    console.log(`[LPGen] Word count info: ${totalWords} words generated (no target set)`);
    return { sections: validSections, autoGeneratedAuthor: Object.keys(autoGeneratedAuthor).length > 0 ? autoGeneratedAuthor : null, wordCountWarning: null };
  }
 ...
 */
export function extractTemplatePlaceholders(skeletonHtml) {
...
export function getRequiredPlaceholderNames(placeholders = {}, narrativeFrame = '') {
  const requiredSlots = new Set(getRequiredTemplateSlots(placeholders, narrativeFrame).map(normalizeTemplateSlotName));
  return uniqueLowerArray(
    ensureArray(placeholders.all)
      .map(normalizeTemplateSlotName)
      .filter((name) => {
        const placeholderRequirement = normalizePlaceholderRequirement(name);
        return placeholderRequirement && requiredSlots.has(placeholderRequirement);
      })
  );
}

export function getRemainingPlaceholderNames(html = '') {
  return uniqueLowerArray(
    [...String(html || '').matchAll(/\{\{([^}]+)\}\}/g)].map((match) => match[1].trim())
  );
}
 ...
 */
function validateNoPlaceholders(html, { requiredPlaceholderNames = [] } = {}) {
  const remaining = [...String(html || '').matchAll(/\{\{([^}]+)\}\}/g)];
  const warnings = [];
  const requiredWarnings = [];

  if (remaining.length === 0) {
    return { html, warnings, requiredWarnings };
  }

  const requiredSet = new Set(ensureArray(requiredPlaceholderNames).map(normalizeTemplateSlotName));
  const names = [...new Set(remaining.map((match) => match[1].trim()))];
  const optionalNames = [];

  for (const name of names) {
    if (requiredSet.has(normalizeTemplateSlotName(name))) {
      requiredWarnings.push(name);
    } else {
  ...
  if (optionalNames.length > 0) {
    console.warn(`[LPGen] Stripping ${optionalNames.length} optional unfilled placeholder(s): ${optionalNames.join(', ')}`);
    warnings.push(...optionalNames);
  }
  if (requiredWarnings.length > 0) {
    console.warn(`[LPGen] Required placeholders still unresolved after post-process: ${requiredWarnings.join(', ')}`);
  }

  const optionalSet = new Set(optionalNames.map(normalizeTemplateSlotName));
  const strippedHtml = optionalSet.size > 0
    ? String(html || '').replace(/\{\{([^}]+)\}\}/g, (match, inner) => (
      optionalSet.has(normalizeTemplateSlotName(inner.trim())) ? '' : match
    ))
    : html;

  return { html: strippedHtml, warnings, requiredWarnings };
}
 ...
 */
export function postProcessLP(html, { project = null, agentConfig = null, angle = '', editorialPlan = null, autoGeneratedAuthor = null, avatarText = null, requiredPlaceholderNames = [] } = {}) {
  let processed = html;
  ...
  // 3. Strip unfilled placeholders (zero-tolerance)
  const validation = validateNoPlaceholders(processed, { requiredPlaceholderNames });
  ...
  // Unresolved required placeholders are critical; optional placeholder stripping is informational.
  if (validation.requiredWarnings.length > 0) {
    criticalWarnings.push(`Required placeholders still unresolved: ${validation.requiredWarnings.join(', ')}`);
  }
  if (validation.warnings.length > 0) {
    infoWarnings.push(`Stripped ${validation.warnings.length} optional placeholder(s): ${validation.warnings.join(', ')}`);
  }
  ...
  return {
    ...
    requiredWarnings: validation.requiredWarnings,
    criticalWarnings,
    infoWarnings,
...
export async function generateAutoLP({
  ...
  const requiredPlaceholderNames = getRequiredPlaceholderNames(placeholders, narrativeFrame);
  ...
  // Determine word count: request override > frame-specific default > global default > no target
  let effectiveWordCount = null;
  if (agentConfig) {
    ...
    const frameWordCount = configuredFrameId != null ? Number(frameWordCounts[configuredFrameId]) : null;
    const globalWordCount = Number(agentConfig.default_word_count);
    effectiveWordCount = Number.isFinite(frameWordCount) && frameWordCount > 0
      ? frameWordCount
      : (Number.isFinite(globalWordCount) && globalWordCount > 0 ? globalWordCount : null);
  }

  const variantDirection = parentAutoContext?.variantDirection || '';
  const { sections: copySections, autoGeneratedAuthor, wordCountWarning } = await generateLandingPageCopy({
    ...
    approvedAds,
    messageBrief,
    additionalDirection: variantDirection || undefined,
    autoContext: {
      narrativeFrame,
  ...
  audit('copy', 'generated', `${copySections.length} sections, ${totalWords} words${effectiveWordCount ? ` (target ${effectiveWordCount})` : ''}: ${copySections.map(s => s.type).join(', ')}${autoGeneratedAuthor ? `, author: ${autoGeneratedAuthor.name}` : ''}${wordCountWarning ? ` ⚠️ ${wordCountWarning}` : ''}`);
  ...
  try {
    ...
    const beliefsText = foundationalDocs?.necessary_beliefs ? foundationalDocs.necessary_beliefs.slice(0, 3000) : '';
    const avatarText = foundationalDocs?.avatar ? foundationalDocs.avatar.slice(0, 2000) : '';
    const qualityCheckPrompt = `You are a direct response copywriting quality reviewer specializing in belief-driven advertorials. Rate this landing page copy on a 1-5 scale.
...
1. BELIEF COVERAGE — Does the copy address the necessary beliefs from the foundational docs? Which beliefs are covered vs missing?
2. AVATAR JOURNEY ARC — Does the reader move through the emotional stages: Awareness (recognizing the problem) → Frustration (failed solutions) → Desperation (breaking point) → Relief (product as the answer)?
3. SPECIFICITY — Does it use concrete product details, stats, mechanisms, and evidence? (not vague platitudes or generic filler)
4. ANGLE ALIGNMENT — Does the copy stay tightly on the marketing angle throughout? (not drifting to generic health/wellness talk)
5. TRUST ARCHITECTURE — Are there testimonials, credentials, guarantees, or social proof elements that build credibility?

Respond with JSON: { "score": 1-5, "weaknesses": ["brief weakness 1", "weakness 2"], "uncovered_beliefs": ["belief not addressed 1", "belief not addressed 2"] }
Score guide: 1=terrible/generic, 2=weak/misaligned, 3=adequate, 4=good, 5=excellent
For "uncovered_beliefs", list any necessary beliefs from the document that the copy fails to address or only addresses superficially.`;
    ...
    if (qualityResult?.score) {
      const uncoveredBeliefsLog = qualityResult.uncovered_beliefs?.length ? `, uncovered beliefs: ${qualityResult.uncovered_beliefs.join('; ')}` : '';
      console.log(`[LPGen] Copy quality gate: score ${qualityResult.score}/5${qualityResult.weaknesses?.length ? `, weaknesses: ${qualityResult.weaknesses.join('; ')}` : ''}${uncoveredBeliefsLog}`);
      audit('quality_gate', qualityResult.score <= 2 ? 'low_quality' : 'passed',
        ...
        qualityResult.uncovered_beliefs?.length ? { uncovered_beliefs: qualityResult.uncovered_beliefs } : {});
      ...
      if (qualityResult.score <= 2) {
        ...
        try {
          ...
          const uncoveredBeliefsFeedback = qualityResult.uncovered_beliefs?.length
            ? ` UNCOVERED BELIEFS: The following necessary beliefs were not addressed: ${qualityResult.uncovered_beliefs.join(', ')}. Weave these beliefs into the copy naturally.`
            : '';
          const retryResult = await generateLandingPageCopy({
            projectId,
            angle,
            ...
            approvedAds,
            messageBrief,
            additionalDirection: `QUALITY FEEDBACK FROM REVIEW: ${weaknessText}.${uncoveredBeliefsFeedback} Address these weaknesses. Make the copy more specific, persuasive, and tightly aligned with the marketing angle "${angle}". Ensure the reader's emotional arc moves through awareness, frustration, desperation, and relief.`,
            autoContext: {
              narrativeFrame,
  ...
  let { html: assembledHtml, warnings, requiredWarnings, criticalWarnings, infoWarnings, hasCriticalIssues } = postProcessLP(rawAssembledHtml, {
    project,
    agentConfig,
    angle,
    editorialPlan,
  ...
  let missingRequiredAfterPostProcess = uniqueLowerArray(
    ensureArray(requiredWarnings).map((name) => normalizePlaceholderRequirement(name)).filter(Boolean)
  ).filter((slot) => requiredStructuralSlots.includes(slot));
  if (missingRequiredAfterPostProcess.length > 0) {
    ...
    try {
      ...
      if (stillMissing.length === 0) {
        ...
        ({ html: assembledHtml, warnings, requiredWarnings, criticalWarnings, infoWarnings, hasCriticalIssues } = postProcessLP(rebuiltHtml, {
          project,
          agentConfig,
          angle,
          editorialPlan,
        ...
        missingRequiredAfterPostProcess = uniqueLowerArray(
          ensureArray(requiredWarnings).map((name) => normalizePlaceholderRequirement(name)).filter(Boolean)
        ).filter((slot) => requiredStructuralSlots.includes(slot));
      } else {
...
/**
 * Compare a generated LP against a canonical page for structural pattern matching.
 * Uses Claude Sonnet to evaluate structural similarity (section order, CTA placement, tone, visual density).
 *
 * @param {object} params
 * @param {string} params.canonicalUrl - URL of the canonical/reference page
 * @param {string} params.generatedHtml - The generated LP's assembled HTML
 * @param {string} params.projectId - Project ID for cost tracking
 * @returns {Promise<{ benchmark_match: number, structural_notes: string[] }>}
 */
export async function runCanonicalBenchmark({ canonicalUrl, generatedHtml, projectId }) {
  if (!canonicalUrl) return { benchmark_match: null, structural_notes: [] };

  let canonicalContent = '';
  try {
    const { fetchSwipePage } = await import('./lpSwipeFetcher.js');
    const captured = await fetchSwipePage(canonicalUrl);
    canonicalContent = captured.textContent || captured.html || '';
    if (!canonicalContent || canonicalContent.length < 200) {
      return { benchmark_match: null, structural_notes: ['Canonical page content too short or empty'] };
    }
  } catch (err) {
    console.warn(`[LPGen] Failed to fetch canonical page for benchmark: ${err.message}`);
    return { benchmark_match: null, structural_notes: [`Failed to fetch canonical: ${err.message}`] };
  }

  // Strip HTML tags for text comparison, truncate both
  const stripTags = (html) => html.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim();
  const canonicalText = stripTags(canonicalContent).slice(0, 6000);
  const generatedText = stripTags(generatedHtml).slice(0, 6000);

  const prompt = `You are an LP structural analyst. Compare these two landing pages and rate how well the GENERATED page matches the CANONICAL page's structural patterns.

CANONICAL PAGE (reference):
${canonicalText}

GENERATED PAGE (to evaluate):
${generatedText}

Rate structural pattern match on a 1-5 scale:
1. SECTION ORDER — Does the generated page follow a similar section flow? (hero → problem → solution → proof → CTA)
2. CTA PLACEMENT — Are CTAs positioned similarly (frequency, location within flow)?
3. TONE & VOICE — Does the generated page match the persuasion style and formality level?
4. CONTENT DENSITY — Similar paragraph lengths, use of subheadings, visual break patterns?
5. PROOF ARCHITECTURE — Similar use of testimonials, stats, expert citations, guarantees?

Respond with JSON: { "benchmark_match": 1-5, "structural_notes": ["note 1", "note 2"] }
Score guide: 1=completely different structure, 2=loosely similar, 3=recognizably similar pattern, 4=closely matches, 5=near-identical structure`;

  try {
    const response = await chat(
      [{ role: 'user', content: prompt }],
      'claude-sonnet-4-6',
      { max_tokens: 1024, timeout: 30000, operation: 'lp_canonical_benchmark', projectId, response_format: { type: 'json_object' } }
    );

    const result = typeof response === 'object' ? response : JSON.parse(response);
    return {
      benchmark_match: result.benchmark_match || null,
      structural_notes: result.structural_notes || [],
    };
  } catch (err) {
    console.warn(`[LPGen] Canonical benchmark comparison failed: ${err.message}`);
    return { benchmark_match: null, structural_notes: [`Benchmark comparison failed: ${err.message}`] };
  }
}
