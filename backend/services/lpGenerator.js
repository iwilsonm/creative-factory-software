...
export async function generateAutoLP({
  ...
  // Mark Builds Brands SOP has no word-count target — `agentConfig.default_word_count`
  // and `agentConfig.frame_word_counts` are intentionally not read.
  const variantDirection = parentAutoContext?.variantDirection || '';
  const { sections: copySections, autoGeneratedAuthor } = await generateLandingPageCopy({
    projectId,
    angle,
    angleBrief,
  ...
  audit('copy', 'generated', `${copySections.length} sections, ${totalWords} words: ${copySections.map(s => s.type).join(', ')}${autoGeneratedAuthor ? `, author: ${autoGeneratedAuthor.name}` : ''}`);
