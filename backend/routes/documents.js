...
// Get the latest headline reference document for a project
router.get('/:projectId/headline-reference', async (req, res) => {
  try {
    const doc = await getLatestDoc(req.params.projectId, 'headline_reference');
    res.json(doc || null);
  } catch (err) {
    console.error('[HeadlineRef] Get error:', err.message);
    res.status(500).json({ error: err.message || 'Failed to get headline reference' });
  }
});

// Upload / update headline reference document
router.post('/:projectId/headline-reference', async (req, res) => {
  try {
    const project = await getProject(req.params.projectId);
    if (!project) return res.status(404).json({ error: 'Project not found' });
    ...
    const existing = await getLatestDoc(req.params.projectId, 'headline_reference');
    const version = existing ? existing.version + 1 : 1;
    const id = uuidv4();
    ...
    await convexClient.mutation(api.foundationalDocs.create, {
      externalId: id,
      project_id: req.params.projectId,
      doc_type: 'headline_reference',
      content: content.trim(),
      version,
      source: 'uploaded',
    });

    res.json({ id, version, charCount: content.trim().length });
  } catch (err) {
    console.error('[HeadlineRef] Upload error:', err.message);
    res.status(500).json({ error: err.message || 'Failed to save headline reference' });
  }
});

// Delete all headline reference documents for a project
router.delete('/:projectId/headline-reference', async (req, res) => {
  try {
    const allDocs = await getDocsByProject(req.params.projectId);
    const headlineRefs = allDocs.filter(d => d.doc_type === 'headline_reference');

    for (const doc of headlineRefs) {
      await convexClient.mutation(api.foundationalDocs.remove, {
        externalId: doc.externalId,
      });
    }

    res.json({ success: true, deleted: headlineRefs.length });
  } catch (err) {
    console.error('[HeadlineRef] Delete error:', err.message);
    res.status(500).json({ error: err.message || 'Failed to delete headline reference' });
  }
});
